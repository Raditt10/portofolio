    ...
    >
      ...
      <div className="relative h-48 sm:h-56 overflow-hidden z-10">
        <div className={`absolute inset-0 z-10 transition-all duration-500 ${
            isLight ? 'bg-black/5 group-hover:bg-transparent' : 'bg-black/30 group-hover:bg-transparent'
        }`} />
        
        <img 
          src={`/img/${gambar}`} 
          alt={judul}
          className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 md:grayscale-0"
          loading="lazy"
        />
        
        {/* Coming Soon Badge - More minimalist */}
        {isComingSoon && (
          <div className="absolute top-4 right-4 z-20">
            <span className={`px-3 py-1 rounded-full text-[10px] font-medium tracking-wider uppercase border backdrop-blur-md ${
              isLight 
      ...
      <div className="flex-1 p-6 flex flex-col z-10">
        <div className="mb-5">
            <h3 className={`text-lg font-semibold mb-2 tracking-tight group-hover:translate-x-1 transition-transform duration-300 ${isLight ? "text-gray-900" : "text-white"}`}>
            {judul}
            </h3>
            <p className={`text-sm line-clamp-2 leading-relaxed font-normal ${isLight ? "text-gray-600" : "text-gray-400"}`}>
            {parag}
            </p>
        </div>
